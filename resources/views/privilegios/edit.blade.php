        ...
        <div class="content flex flex-col">
            <div class="py-4">
                <span class="text-pretty text-xs font-light">
                    Aquí podrás editar un privilegio existente, recuerda que los privilegios son importantes para la
                    seguridad
                    del sistema,
                    por lo que debes tener cuidado al editar privilegios.
                </span>
            </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="relative">
                        <select id="Funcion" name="Funcion"
                            ...
                            <?php
                            $explode = explode(' ', $privilegio->Funcion);
                            $funcion = $explode[0];
                            $caracteristica = $explode[1];
                            ?>
                            <option disabled selected>Selecciona una función</option>
                            <option value="ver" @if ($funcion == 'ver') selected @endif>Ver</option>
                            <option value="crear" @if ($funcion == 'crear') selected @endif>Crear</option>
                            <option value="editar" @if ($funcion == 'editar') selected @endif>Editar</option>
                            <option value="eliminar" @if ($funcion == 'eliminar') selected @endif>Eliminar</option>
                        </select>
                        <div
                    ...
                    <div class="relative">
                        <select id="Caracteristica" name="Caracteristica"
                            ...
                            @foreach (\App\Http\Controllers\UtilController::getModelString() as $name => $value)
                                <option value="{{ $value }}" @if ($caracteristica == $value) selected @endif>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                            ...
                            <div class="pb-2 flex items-center gap-4">
                                <label class="flex items-center cursor-pointer gap-2">
                                    @if ($rol_privilegios->contains($rol->id))
                                        <input type="checkbox" name="rol[{{ $rol->id }}]" value="{{ $rol->id }}"

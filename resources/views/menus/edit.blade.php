        ...
        <div class="content flex flex-col">
            <div class="py-4">
                <span class="text-pretty text-xs font-light">
                    Aquí podrás modificar un menú existente. Actualiza la fecha y ajusta los productos o ingredientes que forman parte del menú, especificando las cantidades necesarias.
                </span>
            </div>
            <form action="{{ route('menus.update', $menu->id) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="date" name="fecha" id="fecha" value="{{ old('fecha', $menu->fecha) }}" required
                                ...
                                <div class="flex items-start border-b pb-3">
                                    <div class="flex items-center h-5 mt-1">
                                        <input type="checkbox"
                                               ...
                                               {{ array_key_exists($inventario->id, old('inventarios', $selected)) ? 'checked' : '' }}>
                                    </div>
                                    <div class="ml-3 w-full">
                                        ...
                                        <div class="mt-1 flex items-center">
                                            ...
                                            <input type="number" 
                                                   ...
                                                   min="1" 
                                                   value="{{ old('inventarios.'.$inventario->id.'.cantidad', $selected[$inventario->id]['cantidad'] ?? 1) }}"
                                                   class="cantidad-input w-20 px-2 py-1 rounded-lg border outline-1 
                                                          -outline-offset-1 outline-brown-300 focus:outline-2 
                                                          focus:outline-orange-700 sm:text-sm/6 disabled:opacity-50"
                                                   {{ array_key_exists($inventario->id, old('inventarios', $selected)) ? '' : 'disabled' }}>
                                        </div>

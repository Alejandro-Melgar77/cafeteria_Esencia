            ...
            <form action="{{ route('nota_compra.store') }}" method="POST">
                ...
                <div class="mb-4">
                    <label for="proveedor_id"
                        class="mb-2 block text-base font-medium text-gray-800 dark:text-gray-200">Proveedor</label>
                    <select name="proveedor_id" id="proveedor_id"
                        class="w-full rounded-lg px-3 py-2 outline-1 outline-brown-300 focus:outline-2 focus:outline-orange-700">
                ...
                <div class="mb-4">
                    <label for="metodo_pago_id"
                        class="mb-2 block text-base font-medium text-gray-800 dark:text-gray-200">Método de Pago</label>
                    ...
                    <div class="item-row mb-4 flex flex-wrap gap-2">
                        ...
                        <select name="items[0][id]" class="rounded-lg px-2 py-1">
                            @foreach ($productos as $producto)
                                <option value="{{ $producto->inventarios->id }}">{{ $producto->inventarios->nombre }}
                                </option>
                            @endforeach
                            @foreach ($ingredientes as $ingrediente)
                                <option value="{{ $ingrediente->inventarios->id }}">{{ $ingrediente->inventarios->nombre }}
                                </option>
                            @endforeach
                        </select>
                        <input type="number" name="items[0][cantidad]" placeholder="Cantidad"
                            class="rounded-lg px-2 py-1 w-28">
                        <input type="number" name="items[0][precio_unitario]" placeholder="Precio Unitario"
                            class="rounded-lg px-2 py-1 w-36">
                    </div>
                </div>

                <button type="button" class="mb-4 px-4 py-2 bg-brown-500 text-white rounded-lg hover:bg-brown-600"
                    onclick="agregarItem()">
                    + Agregar ítem
                </button>
